...
__doc__ = """
*** MAKE SURE YOU ARE ON A FLOOR PLAN VIEW/ DESIRED LEVEL***
...
--------------------------------------------
V2. 26 Jan 2024 -optimized code for built-in
v1: Oct 31 2023
...
ask_user = forms.alert("This script will update ALL the rooms that will have an RLS documentation and will take a long time.\n\n"
                       "Do you want to proceed?", warn_icon=False, exitscript=False, ok=False, yes=True, no=True)
if not ask_user:
    sys.exit()
else:
    all_rooms_in_model = FilteredElementCollector(doc, active_view.Id)\
    ...
    for rm in all_rooms_in_model:
        if rm.Location:
            class_rm = rm.LookupParameter('Rooms_Classification_BLP').AsString()
            if class_rm == 'DEPARTMENTAL - BLP' or class_rm == 'REPEATABLE - BLP':
                selected_rooms.append(rm)
    ...
    with Transaction(doc, __title__) as t:
        ...
        for index, selected_room in enumerate(selected_rooms, start=1):
            ...
            for num, item in enumerate(all_elements, start=1):
                element_collection(item, num, selected_room)

            # ---------------------------------------------xxx----------------------------------------------------
            room_name = selected_room.LookupParameter('Name')

            print('=' * 50)
            print('{}. ROOM NAME : {}'.format(str(index).zfill(3), room_name.AsValueString().upper()))
